...
const Users = (props) => {
    ...
    if (props.users.length === 0) {
        ...
        axios.get('https://social-network.samuraijs.com/api/1.0/users?page=10&count=5').then(response => {
            ...
            {props.users.map(us => (
                <div key={us.id} className={s.item}>
                    <span className={s.ib}>
                        ...
                        <button onClick={() => toggleFollowUser(us.id, us.followed)}> {us.followed ? 'Unfollow' : 'Follow'} </button>
                    </span>
